function [gini, sumcom] = vertiportselecfunc(delthresh, budget, writeoutput, plotoutput)
% close all; clear; clc

addpath('C:\Program Files\Mosek\10.1\toolbox\r2017a')
addpath(genpath('C:\Users\azeem\Documents\YALMIP-master'))
...
constr = [E*y == zeros(nn, 1), ...
    ...
    v <= ones(nc, 1).*wthresh, ...
    ones(nc, 1).*wthresh <= x,... 
    K*y <= 500.*b, ... %vertiport constraints
    sum(b) <= budget];
